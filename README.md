...
- **Liquidation (off-chain service worker)**: **deferred, concrete design ratified.** See `docs/liquidation-design.md` §Track A design. Chosen scheme: pre-published `loan_nullifier = Poseidon(sk, borrow_commitment)` written to a `LoanNullifier(loan_commit)` storage sidecar at borrow-time so a liquidator can trigger without ever seeing the borrower's `sk`. Migration is zero-op: the sidecar is absent for pre-A bonds, and `liquidate_shielded` falls back to the current sk-binding circuit for them. Revised estimate ~3 sessions (was 3–5).
