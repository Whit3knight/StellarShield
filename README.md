...
- **Liquidation (off-chain service worker)**: **deferred.** The v1 nullifier binding uses the borrower's wallet Poseidon `sk`; sharing that key to a liquidation service drains every other note the borrower owns. Two fix tracks scoped in `docs/liquidation-design.md` — Zcash sapling-style ivk/nk key split (~3–5 sessions, cheaper) or threshold decryption (~5–8 sessions). None of the Track L infrastructure is wasted; both follow-ups build on it.
