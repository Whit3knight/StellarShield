...
- **Interest accrual on repay**: **shipped.** `borrow_shielded` snapshots `borrow_index` into a per-loan `BorrowIndexAtOpen(loan_commit)` sidecar; extended repay circuit constrains `deposit_amount × snapshot >= loan_amount × index_now`. Pre-D loans grandfather with `snapshot = index_now` (zero-interest treatment). See commits `ed713b8` + `5f6abd1`.
- **Other deferred**: integration tests on the full liquidation lifecycle against testnet.
